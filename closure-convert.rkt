...
(define (remove-varargs e vst)
  (define emsg0 (gensym 'emsg0))
  (define emsg1 (gensym 'emsg1))
  (define (remove-varargs-h e)
    (match e
      [`(let ([,x ',dat]) ,e0)
       `(let ([,x ',dat]) ,(remove-varargs-h e0))]
      [`(let ([,x (prim ,op ,xs ...)]) ,e0)
       `(let ([,x (prim ,op ,@xs)]) ,(remove-varargs-h e0))]
      [`(let ([,x (apply-prim ,op ,y)]) ,e0)
       `(let ([,x (apply-prim ,op ,y)]) ,(remove-varargs-h e0))]
                  ...
                  (cons gx
                        `(let ([,x (prim car ,gx)])
                           (let ([,head (prim cdr ,gx)])
                             (let ([,n? (prim null? ,head)])
                               (if ,n? ,@(if (not (eq? head na))
                                             `((let ([,h (prim halt ,emsg0)])
                                                 (,h ,h))
                                               ,(cdr gx+e))
                                             `(,(cdr gx+e)
                                               (let ([,h (prim halt ,emsg1)])
                                                 (,h ,h))))))))))
                (cons na (remove-varargs-h body))
                xs))
       `(let ([,x (lambda (,(car gx+e)) ,(cdr gx+e))])
          ,(remove-varargs-h e0))]
      [`(let ([,x (lambda (,xs ...) ,body)]) ,e0)
       `(let ([,x (lambda (,@xs) ,(remove-varargs-h body))])
          ,(remove-varargs-h e0))]
      [`(let ([,x (lambda ,y ,body)]) ,e0)
       `(let ([,x (lambda (,y) ,(remove-varargs-h body))])
          ,(remove-varargs-h e0))]
      [`(if ,x ,e0 ,e1)
       `(if ,x ,(remove-varargs-h e0) ,(remove-varargs-h e1))]
      [`(apply ,f ,args)
       `(,f ,args)] 
      [`(,f ,xs ...)
      ...
      [`(,f ,xs ...)
       `(,f . ,xs)]))
  `(let ([,emsg0 '"library runtime error: Too few arguments provided to function."])
     (let ([,emsg1 '"library runtime error: Too many arguments provided to function."])
       ,(remove-varargs-h e))))
...
(define (proc->llvm procs)
  ...
  (define (wrap-var-alloca v call ptr?)
    (define vptr (gensym 'vptr))
    (if ptr? 
        (string-append
         "  %" (s-> vptr) " = alloca i64*, align 8 \n"
         call " \n "
         " store volatile i64* %" (s-> v) ", i64** %" (s-> vptr) ", align 8 ")
  ...
  (define (e->llvm e)
    (match e
      [`(let ([,x '#t]) ,e0)
       (string-append
        (comment-line
         (wrap-var-alloca x (string-append "  %" (s-> x) " = call i64 @const_init_true()") #f)
         "quoted #t")
        (e->llvm e0))]
      [`(let ([,x '#f]) ,e0)
       (string-append
        (comment-line
         (wrap-var-alloca x (string-append "  %" (s-> x) " = call i64 @const_init_false()") #f)
         "quoted #f")
        (e->llvm e0))]
      [`(let ([,x '()]) ,e0)
       (string-append
        (comment-line
         (wrap-var-alloca x (string-append "  %" (s-> x) " = add i64 0, 0") #f)
         "quoted ()")
        (e->llvm e0))]
      [`(let ([,x ',(? integer? dat)]) ,e0)
       (string-append
        (comment-line
         (wrap-var-alloca x (string-append "  %" (s-> x) " = call i64 @const_init_int(i64 " (number->string dat) ")") #f)
         "quoted int")
        (e->llvm e0))]
      [`(let ([,x ',(? string? dat)]) ,e0)
       ...
       (string-append
        (comment-line
         (wrap-var-alloca
          x
          (string-append "  %" (s-> x) " = call i64 @const_init_string(i8* getelementptr inbounds ("
                         lenstr ", " lenstr "* @" (s-> dx) ", i32 0, i32 0))")
          #f)
         "quoted string")
        (e->llvm e0))]
      [`(let ([,x ',(? symbol? dat)]) ,e0)
       ...
       (string-append
        (comment-line
         (wrap-var-alloca
          x
          (string-append
           "  %" (s-> x) " = call i64 @const_init_symbol(i8* getelementptr inbounds ("
           lenstr ", " lenstr "* @" (s-> dx) ", i32 0, i32 0))")
          #f)
         "quoted string")
        (e->llvm e0))]
      [`(let ([,x (make-closure ,lamx ,xs ...)]) ,e0)
       ...
       (define f (gensym 'f))
       (apply string-append
              ;changes here to emit make_closure call for gc and to change how indices are accessed.
              ;also wrapped all local variables in calls to alloca and store volatile.
              `(,(comment-line
                  (wrap-var-alloca
                   cptr 
                   (string-append "  %" (s-> cptr)
                                  " = call i64* @make_closure(i64 "
                                  (number->string (* (+ (length xs) 1) 8)) ")")
                   #t)
                  "malloc")
                ,@(map (lambda (iptr n)
                         (comment-line
                          (wrap-var-alloca
                           iptr
                           (string-append "  %" (s-> iptr) " = getelementptr inbounds i64, i64* %"
                                          (s-> cptr) ", i64 " (number->string (+ n 1)))
                           #t)
                ...
                ,(comment-line
                  (wrap-var-alloca
                   fptrptr
                   (string-append
                    "  %" (s-> fptrptr) " = getelementptr inbounds i64, i64* %"
                    (s-> cptr) ", i64 1")
                   #t)
                ...
                ,(comment-line
                  ...
                  (wrap-var-alloca
                   f
                   (string-append "  %" (s-> f) " = ptrtoint void(i64"
              ...
              `(,(comment-line
                ...
                ,(comment-line
                  (wrap-var-alloca
                   iptr
                   (string-append
                    "  %" (s-> iptr) " = getelementptr inbounds i64, i64* %"
                    (s-> yptr) ", i64 " (number->string (+ n 1)))
                   #t)
      ...
      [`(if ,x ,e0 ,e1)
       ...
       (define cmpptr (gensym 'cmpptr))
       (string-append
        (comment-line
         "  %" (s-> cmpptr) " = alloca i1, align 8"
         "  %" (s-> cmp) " = icmp eq i64 %" (s-> x) ", 15"
         " store volatile i1 %" (s-> cmp) ", i1* %" (s-> cmpptr) ", align 8" 
         "false?")
      ...
      [`(clo-app ,fx ,xs ...)
       ...
       (define f (gensym 'f))
       (define fptrptr (gensym 'fptrptr))
       (define call-str (foldl string-append "" (map (lambda (_) ",i64") (range (length xs)))))
       (apply string-append
              `(,(comment-line
                  (wrap-var-alloca
                   cloptr
                   (string-append "  %" (s-> cloptr) " = inttoptr i64 %" (s-> fx) " to i64*")
                   #t)
                  "closure/env cast; i64 -> i64*")
                ,(comment-line
                  (wrap-var-alloca
                   i0ptr
                   (string-append "  %" (s-> i0ptr) " = getelementptr inbounds i64, i64* %" (s-> cloptr) ", i64 1")
                   #t)
                  (string-append "&" (s-> cloptr) "[1]"))
                ,(comment-line
                  (wrap-var-alloca
                   f
                   (string-append "  %" (s-> f) " = load i64, i64* %" (s-> i0ptr) ", align 8")
                   #f)
                  (string-append "load; *" (s-> i0ptr)))
                ,(comment-line
                 ;         (pretty-print (string-append "  %" (s-> fptrptr) " = alloca void (i64" call-str ")*, align 8"))
                  ;going to leave this alone for now.
              ;           "  %" (s-> fptrptr) " = alloca i64*, align 8"
                  "  %" (s-> fptr) " = inttoptr i64 %" (s-> f) " to void (i64" call-str ")*"
                  ;  "  store volatile void (i64 " call-str ")* %" fptr ", void (i64" call-str ")** %" fptrptr ", align 8"
             ;     "  store volatile i64* %" fptr ", i64** %" fptrptr ", align 8"
                  "cast fptr; i64 -> void(...)*")
                ,(comment-line
