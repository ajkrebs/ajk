...
//#define MEMORY_LIMIT_BYTES 25600000000
...
extern "C"
{
  ...
  //int, string, symbol, cons
  u64 valid_application(u64 f)
  {
    ASSERT_NE_TAG(f, INT_TAG, "Integer in function position.")
      ASSERT_NE_TAG(f, STR_TAG, "String in function position.")
      ASSERT_NE_TAG(f, CONS_TAG, "Cons Cell in function position.")
      ASSERT_NE_TAG(f, SYM_TAG, "Symbol in function position.")
      return V_TRUE;
  }
  ...
  u64* alloc(const u64 m)
  {
    ...
    //{
    //   fatal_err("Memory limit exceeded.");
    //  }
    //  else
    return (u64*)(malloc(m));
...
*/
  ...
  u64 prim_make_hash()
  {
    const hamt<scheme_key, scheme_value>* h =
      new ((hamt<scheme_key, scheme_value>*)
	   malloc(sizeof(hamt<scheme_key, scheme_value>)))hamt<scheme_key, scheme_value>();
    
    u64* hash = (u64*)alloc(2 * sizeof(u64));
    //need to make the second element a hash_ptr type or something.
  ...
  /*
  u64 prim_hash_set(u64 hash, u64 key, u64 value)
  {
    
    //set key to value in hash and return a new hash.
    return V_NULL;
  }
    
  int prim_hash_ref(u64 h, u64 key)
  {
    u64* hash_object = DECODE_OTHER(h);
    printf("%lu \n", hash_object[0]);
    const hamt<scheme_key,scheme_key>* hash_ptr = (hamt<scheme_key,scheme_key>*)hash_object[1];
    const scheme_key* const s = new ((scheme_key*)malloc(sizeof(scheme_key))) scheme_key(const_init_int(2222), 20);
    printf(" %lu \n", hash_ptr->size());
    //  u64 v = (((hamt<scheme_key, scheme_key>*)hash_object[1])->get(s)->key  );
 
    return DECODE_INT(hash_ptr->get(s)->value);
    //ToDo:
