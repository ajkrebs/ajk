...
(define (t-prim e env)
  (match e
    [`(prim hash-ref ,h ,k ,t)
     `(hash-ref-internal ,(t-desugar h env) ,(t-desugar k env) ,(t-desugar t env))]
    [`(prim hash-ref ,h ,k)
     `(hash-ref-internal ,(t-desugar h env) ,(t-desugar k env) '())]
    [`(,xprim ,op ,es ...)
...
(define (desugar e)
  ...
  (define prims-env (foldl (lambda (p env) (hash-set env p #t))
                           aliases-env
                           (prims->list)))
  `(let ([hash-ref-internal
          (lambda (h k thunk)
            (let ([r (prim hash-ref h k)])
              (if (prim eq? r '0)
                  (if (prim null? thunk)
                      (prim halt '"library runtime error: Hash key not found")
                      (thunk))
                  r)))])
